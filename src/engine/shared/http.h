...
public:
	CHttpRequest(const char *pUrl, class CConfig *pConfig);
	~CHttpRequest();
...
inline std::unique_ptr<CHttpRequest> HttpHead(const char *pUrl, class CConfig *pConfig)
{
...
inline std::unique_ptr<CHttpRequest> HttpGet(const char *pUrl, class CConfig *pConfig)
{
	return std::make_unique<CHttpRequest>(pUrl, pConfig);
}

inline std::unique_ptr<CHttpRequest> HttpGetFile(const char *pUrl, class CConfig *pConfig, IStorage *pStorage, const char *pOutputFile, int StorageType)
{
...
inline std::unique_ptr<CHttpRequest> HttpPost(const char *pUrl, class CConfig *pConfig, const unsigned char *pData, size_t DataLength)
{
	auto pResult = std::make_unique<CHttpRequest>(pUrl, pConfig);
...
inline std::unique_ptr<CHttpRequest> HttpPostJson(const char *pUrl, class CConfig *pConfig, const char *pJson)
{
	auto pResult = std::make_unique<CHttpRequest>(pUrl, pConfig);
