...
void *CGameContext::PreProcessMsg(int *pMsgID, CUnpacker *pUnpacker, int ClientID)
{
	if(Server()->ClientProtocol(ClientID) == NETPROTOCOL_SIX)
	{
		...
		{
			protocol6::CNetMsg_Cl_StartInfo *pMsg6 = (protocol6::CNetMsg_Cl_StartInfo *)pRawMsg;
			::CNetMsg_Cl_StartInfo *pMsg = (::CNetMsg_Cl_StartInfo *)s_aRawMsg;

			pMsg->m_pName = pMsg6->m_pName;
			pMsg->m_pClan = pMsg6->m_pClan;
			...
			for(int i = 0; i < 6; i ++)
			{
				pMsg->m_apSkinPartNames[i] = Info.m_apSkinPartNames[i];
				pMsg->m_aSkinPartColors[i] = Info.m_aSkinPartColors[i];
				pMsg->m_aUseCustomColors[i] = Info.m_aUseCustomColors[i];
			}
		}
		else if(*pMsgID == protocol6::NETMSGTYPE_CL_CHANGEINFO)
		{
			// set infos
			if(Config()->m_SvSpamprotection && pPlayer->m_LastChangeInfoTick && pPlayer->m_LastChangeInfoTick+Server()->TickSpeed()*5 > Server()->Tick())
				return 0;

			bool NeedsUpdate = false;

			protocol6::CNetMsg_Cl_ChangeInfo *pMsg = (protocol6::CNetMsg_Cl_ChangeInfo *)pRawMsg;
			...
			{
				char aBuf[128];
				str_format(aBuf, sizeof(aBuf), "'%s' changed name to '%s'", aOldName, Server()->ClientName(ClientID));
				SendChat(-1, CHAT_ALL, -1, aBuf);
			}
		...
		else if(*pMsgID == protocol6::NETMSGTYPE_CL_SETSPECTATORMODE)
		{
			protocol6::CNetMsg_Cl_SetSpectatorMode *pMsg6 = (protocol6::CNetMsg_Cl_SetSpectatorMode *)pRawMsg;
			::CNetMsg_Cl_SetSpectatorMode *pMsg = (::CNetMsg_Cl_SetSpectatorMode *)s_aRawMsg;

			pMsg->m_SpecMode = pMsg6->m_SpectatorID == -1 ? SPEC_FREEVIEW : SPEC_PLAYER;
			pMsg->m_SpectatorID = pMsg6->m_SpectatorID;
		}
		else if(*pMsgID == protocol6::NETMSGTYPE_CL_SETTEAM)
		{
			protocol6::CNetMsg_Cl_SetTeam *pMsg6 = (protocol6::CNetMsg_Cl_SetTeam *)pRawMsg;
			::CNetMsg_Cl_SetTeam *pMsg = (::CNetMsg_Cl_SetTeam *)s_aRawMsg;

			pMsg->m_Team = pMsg6->m_Team;
		}
		else if(*pMsgID == protocol6::NETMSGTYPE_CL_CALLVOTE)
		{
			protocol6::CNetMsg_Cl_CallVote *pMsg6 = (protocol6::CNetMsg_Cl_CallVote *)pRawMsg;
			::CNetMsg_Cl_CallVote *pMsg = (::CNetMsg_Cl_CallVote *)s_aRawMsg;

			pMsg->m_Force = false;
			pMsg->m_Value = pMsg6->m_Value;
			pMsg->m_Reason = pMsg6->m_Reason;
			pMsg->m_Type = pMsg6->m_Type;
		}
		else if(*pMsgID == protocol6::NETMSGTYPE_CL_EMOTICON)
		{
			protocol6::CNetMsg_Cl_Emoticon *pMsg6 = (protocol6::CNetMsg_Cl_Emoticon *)pRawMsg;
			::CNetMsg_Cl_Emoticon *pMsg = (::CNetMsg_Cl_Emoticon *)s_aRawMsg;

			pMsg->m_Emoticon = pMsg6->m_Emoticon;
		}
		else if(*pMsgID == protocol6::NETMSGTYPE_CL_VOTE)
		{
			protocol6::CNetMsg_Cl_Vote *pMsg6 = (protocol6::CNetMsg_Cl_Vote *)pRawMsg;
			::CNetMsg_Cl_Vote *pMsg = (::CNetMsg_Cl_Vote *)s_aRawMsg;

			pMsg->m_Vote = pMsg6->m_Vote;
		}

		*pMsgID = Msg_SixToSeven(*pMsgID);
